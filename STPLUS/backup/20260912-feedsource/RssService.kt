...
internal object RssService {
    ...
    private fun fetchFeeds(channelIds: List<String>): MutableList<MediaItem> = runBlocking {
        ...
        // >>> STPLUS: limit rownoleglosci. Upstream startuje korutyne dla KAZDEGO kanalu
        // naraz (przy 50 kanalach = 50 rownoleglych pobran + parsowan) -> na armv7 glodzi
        // watek glowny i sypie GC. NewPipe pobiera 3 naraz (PARALLEL_EXTRACTIONS).
        // 0 = zachowanie upstreama.
        //
        // UWAGA (blad z 2026-09-12, naprawiony): pierwsza wersja uzywala blokujacego
        // java.util.concurrent.Semaphore wewnatrz launch{}. Te korutyny startuja na
        // JEDNYM watku runBlocking, wiec czwarte acquire() blokowalo ten watek i
        // zwolnienia nigdy nie mialy gdzie sie wykonac => zakleszczenie (feed nie wracal).
        // Zamiast semafora robimy porcje przez chunked() — struktura zamiast blokady.
        val limit = RssOptions.maxParallelChannels
        val chunks = if (limit > 0) channelIds.chunked(limit) else listOf(channelIds)
        // <<< STPLUS

        for (chunk in chunks) {
            coroutineScope { // wait for all child coroutines complete
                for ((index, channelId) in chunk.withIndex()) {
                    launch {
                        // >>> STPLUS: rozsuniecie zapytan w czasie (YouTube dlawi serie -> 404)
                        val step = RssOptions.delayBetweenChannelsMs
                        if (step > 0 && index > 0) {
                            kotlinx.coroutines.delay(step * index)
                        }
                        // <<< STPLUS
    ...
    private suspend fun fetchFeed(channelId: String): List<MediaItem>? = withContext(Dispatchers.IO) {
        try {
            ...
            // >>> STPLUS: syncWithChannel to DRUGIE, ciezkie zapytanie na kazdy kanal
            // (POST youtubei/v1/browse, ogromny JSON). Dodaje badge/live/preview i wycina
            // pozycje nieobecne w gridzie kanalu. Przy feedzie z kilkudziesieciu kanalow
            // to glowny koszt - NewPipe robi wylacznie RSS.
            if (!RssOptions.skipChannelSync) {
